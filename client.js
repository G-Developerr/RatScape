...
window.addEventListener('beforeunload', function() {
    ...
    // ===== BACKGROUND SLIDESHOW SYSTEM - FIXED =====
...
function initBackgroundSlideshow() {
    ...
    // Δημιουργία preloader για τις εικόνες
    preloadBackgrounds();
    
    // Ξεκίνημα του slideshow
    startBgSlideshow();
    
    // Προσθήκη event listeners για διακοπή όταν η σελίδα δεν είναι ορατή
...
function startBgSlideshow() {
    ...
    console.log('🔄 Starting slideshow (10 second intervals)...');
...
function changeBackground(imageName) {
    ...
    // Smooth fade effect με transition
    body.style.transition = 'background 1.5s ease-in-out';
    body.style.background = `linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('${imageName}') center/cover fixed`;
    
    console.log(`🖼️ Background changed to: ${imageName} at ${new Date().toLocaleTimeString()}`);
}

function handleVisibilityChange() {
    if (document.hidden) {
        // Παύση όταν η σελίδα δεν είναι ορατή
        if (bgInterval) {
            clearInterval(bgInterval);
            bgInterval = null;
            console.log('⏸️ Background slideshow paused (page hidden)');
        }
    } else {
        // Επανεκκίνηση όταν η σελίδα γίνεται ορατή ξανά
        if (!bgInterval) {
            startBgSlideshow();
            console.log('▶️ Background slideshow resumed');
        }
    }
}

// Στο DOMContentLoaded event, πρόσθεσε:
document.addEventListener("DOMContentLoaded", async () => {
    console.log("🐀 RatScape client initialized");
...
// 🔥 DEBUG: Προσθήκη κουμπιού για testing
setTimeout(() => {
    const debugBtn = document.createElement('button');
    debugBtn.innerHTML = '🖼️ Test BG';
