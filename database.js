...
// Προσθήκη στο database.js μετά τις υπάρχουσες event functions

// 🔥 ΝΕΟ: Δημιουργία room για event
createRoomForEvent: async function(eventId, eventTitle, createdBy) {
    const inviteCode = `EVENT_${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
    const roomId = `event_room_${eventId}`;
    ...
    const room = new Room({
        ...
        invite_code: inviteCode,
        created_by: createdBy,
        is_event_room: true, // 🔥 Νέο πεδίο για να ξεχωρίζουμε τα event rooms
        event_id: eventId // 🔥 Σύνδεση με το event
    });
    
    await room.save();
    
    // Προσθήκη του δημιουργού στο room
    await this.addUserToRoom(roomId, createdBy);
    
    console.log(`✅ Event room created: ${roomId} for event ${eventId}`);
    return { roomId, inviteCode };
},

// 🔥 ΝΕΟ: Λήψη room για event
getRoomForEvent: async function(eventId) {
    const room = await Room.findOne({ event_id: eventId });
    if (room) {
        return {
            id: room.room_id,
            name: room.name,
...
// 🔥 ΝΕΟ: Προσθήκη event participant στο room
addEventParticipantToRoom: async function(eventId, username) {
    const room = await Room.findOne({ event_id: eventId });
    if (room) {
        await this.addUserToRoom(room.room_id, username);
        console.log(`✅ ${username} added to event room ${room.room_id}`);
        return true;
    }
    return false;
},

// 🔥 ΝΕΟ: Αφαίρεση event participant από room
removeEventParticipantFromRoom: async function(eventId, username) {
    const room = await Room.findOne({ event_id: eventId });
    if (room) {
        await this.removeUserFromRoom(room.room_id, username);
        console.log(`✅ ${username} removed from event room ${room.room_id}`);
        return true;
    }
    return false;
},

// 🔥 ΝΕΟ: Έλεγχος αν ο χρήστης είναι στο event room
isUserInEventRoom: async function(eventId, username) {
    const room = await Room.findOne({ event_id: eventId });
    if (room) {
        return await this.isUserInRoom(room.room_id, username);
    }
    return false;
},

// 🔥 ΝΕΟ: Ανανέωση της createEvent για αυτόματη δημιουργία room
createEventWithRoom: async function(eventData) {
    // 1. Δημιουργία event
    const event = await this.createEvent(eventData);
    
    // 2. Δημιουργία room για το event
    const roomData = await this.createRoomForEvent(
        event.event_id, 
        eventData.title, 
        eventData.created_by
    );
    
    // 3. Ενημέρωση του event με το room ID
    await Event.updateOne(
        { event_id: event.event_id },
        { room_id: roomData.roomId }
    );
    
    // 4. Επιστροφή και event και room data
    return {
        event: event,
        room: roomData
    };
...
// 🔥 ΝΕΟ: Ανανέωση της joinEvent για αυτόματη προσθήκη στο room
joinEventWithRoom: async function(eventId, username) {
    // 1. Join στο event
    const event = await this.joinEvent(eventId, username);
    
    if (event) {
        // 2. Join στο room
        await this.addEventParticipantToRoom(eventId, username);
    }
    
    return event;
},

// 🔥 ΝΕΟ: Ανανέωση της leaveEvent για αυτόματη αφαίρεση από το room
leaveEventWithRoom: async function(eventId, username) {
    // 1. Leave από το event
    const event = await this.leaveEvent(eventId, username);
    
    if (event) {
        // 2. Leave από το room (εξαιρουμένου του δημιουργού)
        if (event.created_by !== username) {
            await this.removeEventParticipantFromRoom(eventId, username);
        }
    }
...
// 🔥 ΝΕΟ: Λήψη event με πληροφορίες room
getEventWithRoomInfo: async function(eventId, username = null) {
    const event = await this.getEventById(eventId);
    if (!event) return null;
    
    const room = await this.getRoomForEvent(eventId);
    const isInRoom = username ? await this.isUserInEventRoom(eventId, username) : false;
    
    return {
        ...event,
        has_room: !!room,
        room_id: room ? room.id : null,
        room_invite_code: room ? room.invite_code : null,
        is_in_room: isInRoom
    };
},

// 🔥 ΝΕΟ: Λήψη όλων των events με room info
getAllEventsWithRoomInfo: async function(username = null) {
    const events = await this.getAllEvents(username);
    
    const eventsWithRooms = await Promise.all(
        events.map(async (event) => {
            const room = await this.getRoomForEvent(event.id);
            const isInRoom = username ? await this.isUserInEventRoom(event.id, username) : false;
...
// 🔥 ΝΕΟ: Διαγραφή event και του αντίστοιχου room
deleteEventWithRoom: async function(eventId, username) {
    const event = await Event.findOne({ event_id: eventId });
    if (!event) {
        throw new Error("Event not found");
    }
    
    const isAdmin = username.toLowerCase() === "vf-rat";
    const isCreator = event.created_by === username;
    ...
    // 1. Βρείτε το room του event
    const room = await Room.findOne({ event_id: eventId });
    
    // 2. Διαγραφή όλων των μελών από το room
    if (room) {
        await RoomMember.deleteMany({ room_id: room.room_id });
    }
    
    // 3. Διαγραφή όλων των μηνυμάτων από το room
    if (room) {
        await Message.deleteMany({ room_id: room.room_id });
    }
    
    // 4. Διαγραφή του room
    if (room) {
        await Room.deleteOne({ room_id: room.room_id });
        console.log(`✅ Event room deleted: ${room.room_id}`);
    }
    
    // 5. Διαγραφή των event participants
    event.participants = [];
    await event.save();
    
    // 6. Διαγραφή του event
    const result = await Event.deleteOne({ event_id: eventId });
    
    console.log(`✅ Event deleted with room: ${event.title} by ${username}`);
    return result.deletedCount > 0;
}
