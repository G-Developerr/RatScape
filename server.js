...
// Προσθήκη στο server.js μετά τα υπάρχοντα event endpoints

// 🔥 ΝΕΟ: Δημιουργία event με room
app.post("/create-event-with-room", validateSession, async (req, res) => {
    try {
        const { title, description, date, location, max_participants, is_public, photo } = req.body;
        const username = req.body.username || req.user?.username;
        ...
        const result = await dbHelpers.createEventWithRoom({
            title,
            description,
            date: new Date(date),
            location,
        ...
        res.json({
            ...
            event: {
                ...
                max_participants: result.event.max_participants,
                participants: result.event.participants,
                is_public: result.event.is_public,
                created_at: result.event.created_at,
                photo: result.event.photo,
                room_id: result.room.roomId,
                room_invite_code: result.room.inviteCode
            },
            message: "Event created with room successfully"
        });
        
        // Ενημέρωση μέσω WebSocket
        io.emit("event_update", {
            type: "event_created_with_room",
        ...
        console.error("❌ Error creating event with room:", error);
        res.status(500).json({ success: false, error: getErrorMessage(error) });
    }
});

// 🔥 ΝΕΟ: Join event με auto-join στο room
app.post("/events/:eventId/join-with-room", validateSession, async (req, res) => {
    try {
        const { eventId } = req.params;
        const username = req.body.username || req.user?.username;
        ...
        const event = await dbHelpers.joinEventWithRoom(eventId, username);
        ...
        res.json({
            ...
            message: "Joined event and room successfully"
        });
        
        // Ενημέρωση μέσω WebSocket
        io.emit("event_update", {
            type: "participant_joined_with_room",
            eventId: eventId,
            username: username,
            participant_count: event.participants.length
        });
        ...
        console.error("❌ Error joining event with room:", error);
        res.status(500).json({ 
            success: false, 
            error: error.message || "Failed to join event" 
        });
...
// 🔥 ΝΕΟ: Leave event με auto-leave από room
app.post("/events/:eventId/leave-with-room", validateSession, async (req, res) => {
    try {
        const { eventId } = req.params;
        const username = req.body.username || req.user?.username;
        ...
        const event = await dbHelpers.leaveEventWithRoom(eventId, username);
        ...
        res.json({
            ...
            message: "Left event and room successfully"
        });
        
        // Ενημέρωση μέσω WebSocket
        io.emit("event_update", {
            type: "participant_left_with_room",
            eventId: eventId,
            username: username,
            participant_count: event.participants.length
        });
        ...
        console.error("❌ Error leaving event with room:", error);
        res.status(500).json({ 
            success: false, 
            error: error.message || "Failed to leave event" 
        });
...
// 🔥 ΝΕΟ: Enter event room
app.post("/events/:eventId/enter-room", validateSession, async (req, res) => {
    try {
        const { eventId } = req.params;
        const username = req.body.username || req.user?.username;
        ...
        // Έλεγχος αν ο χρήστης είναι event participant
        const event = await dbHelpers.getEventById(eventId);
        if (!event) {
            return res.status(404).json({ success: false, error: "Event not found" });
        }
        
        if (!event.participants.includes(username) && event.created_by !== username) {
            return res.status(403).json({ 
                success: false, 
                error: "You must be an event participant to join the room" 
            });
        }
        
        // Βρείτε το room του event
        const room = await dbHelpers.getRoomForEvent(eventId);
        if (!room) {
            return res.status(404).json({ success: false, error: "Event room not found" });
        }
        
        // Προσθέστε τον χρήστη στο room αν δεν είναι ήδη μέσα
        const isInRoom = await dbHelpers.isUserInRoom(room.id, username);
        if (!isInRoom) {
            await dbHelpers.addUserToRoom(room.id, username);
        }
        ...
        res.json({
            ...
            message: "Entered event room successfully"
        });
        
    } catch (error) {
        console.error("❌ Error entering event room:", error);
        res.status(500).json({ 
            success: false, 
            error: error.message || "Failed to enter event room" 
        });
    }
});

// 🔥 ΝΕΟ: Get event with room info
app.get("/events-with-rooms/:username", validateSession, async (req, res) => {
    try {
        const username = req.params.username;
        const events = await dbHelpers.getAllEventsWithRoomInfo(username);
        ...
        console.error("❌ Error getting events with room info:", error);
        res.status(500).json({ success: false, error: getErrorMessage(error) });
    }
});

// 🔥 ΝΕΟ: Διαγραφή event με room
app.delete("/events-with-room/:eventId", validateSession, async (req, res) => {
    try {
        const { eventId } = req.params;
        const username = req.body.username || req.user?.username;
        ...
        const success = await dbHelpers.deleteEventWithRoom(eventId, username);
        
        res.json({
            success: true,
            message: "Event and room deleted successfully"
        });
        
        // Ενημέρωση όλων μέσω WebSocket
        io.emit("event_update", {
            type: "event_and_room_deleted",
            eventId: eventId
        });
        
    } catch (error) {
        console.error("❌ Error deleting event with room:", error);
        res.status(500).json({ 
            success: false, 
            error: error.message || "Failed to delete event" 
        });
